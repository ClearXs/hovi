...
export class MemoryIndexManager extends MemoryManagerEmbeddingOps implements MemorySearchManager {
  ...
  /**
   * Delete a knowledge document from the index
   */
  async deleteKnowledgeDocument(params: { documentId: string }): Promise<void> {
    const pathKey = `knowledge/${params.documentId}`;

    // Delete from vector table
    const deleteChunk = this.db.prepare(`DELETE FROM chunks WHERE path = ? AND source = ?`);
    deleteChunk.run(pathKey, "knowledge");
    ...
    log.info(`knowledge: deleted document ${params.documentId}`);
  }

...
export async function copyA2uiAssets({ srcDir, outDir }: { srcDir: string; outDir: string }) {
  const skipMissing =
    shouldSkipMissingA2uiAssets(process.env) ||
    typeof process.env.OPENCLAW_UI_AGENT_DESKTOP_TARGET === "string";
  try {
    ...
    if (skipMissing) {
      console.warn(
        `${message} Skipping copy because OPENCLAW_A2UI_SKIP_MISSING=1 or OPENCLAW_UI_AGENT_DESKTOP_TARGET set or OPENCLAW_SPARSE_PROFILE is set.`,
      );
      return;
    }

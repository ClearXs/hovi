...
export function getGatewayHttpBaseUrl(opts?: { tauri?: boolean }): string {
  if (opts?.tauri ?? isTauriRuntime()) {
    return DEFAULT_DESKTOP_HTTP_URL;
  }

  return normalizeBaseUrl(process.env.NEXT_PUBLIC_API_URL || DEFAULT_BROWSER_HTTP_URL);
}
...
export function buildGatewayUrl(path: string): string {
  return new URL(path, `${getGatewayHttpBaseUrl()}/`).toString();
}
